  ...
  <div class="grid crud-demo">
    <div class="col-12">
      <div class="card">
        <Toolbar class="mb-4">
          <template v-slot:start>
            <div class="my-2">
              ...
              <Button label="Ingest" icon="pi pi-cloud-download" class="p-button-warning mr-2" @click="openIngest" />
              <Button label="Auto-Ingest" icon="pi pi-cloud-download" class="p-button-info mr-2" @click="autoIngest" />
            </div>
          </template>
          ...
          <Column headerStyle="width:14%; min-width:15rem;">
            <template #body="slotProps">
              ...
              <Button icon="pi pi-pencil" class="p-button-rounded p-button-success mr-2"
                      @click="editProgram(slotProps.data)" />
        ...
        <Dialog v-model:visible="programDialog" :style="{ width: '600px' }" header="Program Details" :modal="true"
                class="p-fluid">
          <img :src="program.picture" :alt="program.picture" v-if="program.picture" width="150"
               class="mt-0 mx-auto mb-5 block shadow-2" />
          <div class="field">
            <label for="ID">ID</label>
            <InputText id="ID" v-model.trim="program.ID" :disabled="program.prvider != 'internal'" required="true"
                       autofocus :class="{ 'p-invalid': submitted && !program.ID }" />
            <small class="p-invalid" v-if="submitted && !program.ID">ID is required.</small>
          </div>
          <div class="field">
            <label for="name">Title</label>
            <InputText id="title" v-model.trim="program.title" :disabled="program.prvider != 'internal'" required="true"
                       autofocus :class="{ 'p-invalid': submitted && !program.title }" />
            <small class="p-invalid" v-if="submitted && !program.title">Title is required.</small>
          </div>

          <div class="field">
            <label for="status" class="mb-3">Status</label>
            <Dropdown id="status" v-model="program.status" :options="statuses" optionLabel="label"
        ...
        <Dialog v-model:visible="programIngestDialog" :style="{ width: '600px' }" header="Program Ingest" :modal="true"
                class="p-fluid">
          <div class="field">
...
export default {
  ...
  methods: {
    ...
    ingestProgram() {
      ...
      this.programService.ingestProgram(this.permalink).then(() => {
        this.$toast.add({
          severity: "success",
          summary: "Successful",
          detail: "Program Ingestion started",
          life: 3000
        });
        this.permalink = null;
        this.programIngestDialog = false;
    ...
    autoIngest() {
      this.submitted = true;
      this.programService.autoIngest().then(() => {
        this.$toast.add({
          severity: "success",
          summary: "Successful",
          detail: "Program auto Ingest started",
          life: 3000
        });
        this.submitted = false;
      });
