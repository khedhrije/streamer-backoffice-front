          ...
          <Column headerStyle="width:14%; min-width:10rem;">
            <template #body="slotProps">
              ...
              <Button icon="pi pi-trash" class="p-button-rounded p-button-warning"
                      @click="confirmDeleteMedia(slotProps.data)" />
            </template>
          </Column>
        </DataTable>

        <Dialog v-model:visible="mediaDialog" :style="{ width: '600px' }" header="Media Details" :modal="true"
                class="p-fluid">
          <img :src="media.picture" :alt="media.picture" v-if="media.picture" width="150"
               class="mt-0 mx-auto mb-5 block shadow-2" />
          <div class="field">
            <label for="ID">ID</label>
            <InputText id="ID" v-model.trim="media.ID" :disabled="media.prvider != 'internal'" required="true" autofocus
                       :class="{ 'p-invalid': submitted && !media.ID }" />
            <small class="p-invalid" v-if="submitted && !media.ID">ID is required.</small>
          </div>
          <div class="field">
            <label for="name">Title</label>
            <InputText id="title" v-model.trim="media.title" :disabled="media.prvider != 'internal'" required="true"
                       autofocus :class="{ 'p-invalid': submitted && !media.title }" />
            <small class="p-invalid" v-if="submitted && !media.title">Title is required.</small>
          </div>
          <div class="field">
            <label for="description">Description</label>
            <Textarea id="description" v-model="media.description" :disabled="media.prvider != 'internal'"
                      required="true" rows="5" cols="20" />
          </div>

          <div class="field">
            <label for="status" class="mb-3">Status</label>
            <Dropdown id="status" v-model="media.status" :options="statuses" optionLabel="label"
...
export default {
  ...
  mounted() {
    this.mediaService.getMediasByProgramUUID(this.program.ID).then((data) => (this.medias = data));
  },
  methods: {
    openNew() {
      this.media = {};
