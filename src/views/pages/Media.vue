  ...
  <div>
    <div class="grid grid-nogutter surface-section text-800">
      ...
      <div class="col-12 md:col-6 overflow-hidden">
        <img :src="media.picture" alt="Image" class="md:ml-auto block md:h-full"
             style="max-height: 25rem; clip-path: polygon(8% 0, 100% 0%, 100% 100%, 0 100%)" />
      </div>
        ...
        <div class="grid grid-nogutter">
          <div class="col-12">
            <div class="flex flex-column sm:flex-row sm:align-items-center p-4 gap-3"
                 :class="{ 'border-top-1 surface-border': media }">
                ...
                <div class="flex flex-row md:flex-column justify-content-between align-items-start gap-2">
                  ...
                  <div>
                    <span class="font-medium text-secondary text-sm">{{ formatTime(currentTime)
                      }} / {{ formatTime(media.duration) }}</span>
                  </div>
                  <input v-if="media.ID === media.ID" style="min-width: 100%" type="range" :value="currentTime"
                         :max="duration" @input="seek($event.target.value)" />
                  <p></p>
                </div>
                <div class="flex flex-column md:align-items-end gap-5">
                  <div class="flex flex-row-reverse md:flex-row gap-2">
                    <!-- Play button only shows if media is not playing or a different media is playing -->
                    <Button v-if="!isPlaying || media.ID !== media.ID" icon="pi pi-play" outlined
                            @click="play(media)"></Button>
                    <!-- Pause button shows only when this specific media is playing and is not paused -->
                    <Button v-if="media.ID === media.ID && isPlaying && !isPaused" icon="pi pi-pause" outlined
                            @click="pause()"></Button>
                  </div>
                </div>
              </div>
