<template>
  <div id="layout-config">
    ...
    <div class="layout-config" :class="{ 'layout-config-active': configActive }" @click="onConfigClick">
      ...
      <div class="field-radiobutton">
        <RadioButton id="horizontal" name="layoutMode" value="horizontal" v-model="d_layoutMode"
                     @change="changeLayout($event, 'horizontal')" />
        <label for="horizontal">Horizontal</label>
      </div>
      <div class="field-radiobutton">
        <RadioButton id="slim" name="layoutMode" value="slim" v-model="d_layoutMode"
                     @change="changeLayout($event, 'slim')" />
        <label for="slim">Slim</label>
      </div>

      <h5>Menu Colors</h5>
      <div class="field-radiobutton">
        <RadioButton id="dark" name="lightMenu" :value="true" v-model="d_darkMenu"
                     @change="changeMenuColor($event, true)" />
        <label for="dark">Dark</label>
      </div>
      <div class="field-radiobutton">
        <RadioButton id="light" name="lightMenu" :value="false" v-model="d_darkMenu"
                     @change="changeMenuColor($event, false)" />
        <label for="light">Light</label>
      </div>

      <h5>User Profile</h5>
      <div class="field-radiobutton">
        <RadioButton id="inline" name="profileMode" value="inline" v-model="d_profileMode" :disabled="isHorizontal()"
                     @change="changeProfileMode($event, 'inline')" />
        <label for="dark">Inline</label>
      </div>
      <div class="field-radiobutton">
        <RadioButton id="popup" name="profileMode" value="popup" v-model="d_profileMode" :disabled="isHorizontal()"
                     @change="changeProfileMode($event, 'popup')" />
        <label for="dark">Popup</label>
      </div>

      <h5 style="margin-top: 0">Input Style</h5>
      <div class="field-radiobutton">
        <RadioButton id="input_outlined" name="inputstyle" value="outlined" :modelValue="value"
                     @update:modelValue="onChange" />
        <label for="input_outlined">Outlined</label>
      </div>
      <div class="field-radiobutton">
        <RadioButton id="input_filled" name="inputstyle" value="filled" :modelValue="value"
                     @update:modelValue="onChange" />
      ...
      <div class="layout-themes">
        <div v-for="t of themes" :key="t.name">
          <a @click="changeComponentTheme($event, t.file)"
             :class="['layout-config-option', { selected: theme === t.file }]" :style="{ backgroundColor: t.color }">
